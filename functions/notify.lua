--[[
	...
	local Notify = loadstring(game:HttpGet("https://raw.githubusercontent.com/sinsly/user-interface/main/functions/notify.lua"))()
	Notify.New("X", Y)
...
function Notif.New(text, timee)
    if #activeNotifs >= MAX_STACK then
        -- Remove oldest notification if max stack reached
    ...
    table.insert(activeNotifs, frame_2)

    -- Close function
    local function close_notif()
        TS:Create(image_button, TweenInfo.new(0.15), { ImageTransparency = 1 }):Play()
        TS:Create(text_button, TweenInfo.new(0.15), { BackgroundTransparency = 1 }):Play()
        TS:Create(text_label, TweenInfo.new(0.15), { TextTransparency = 1 }):Play()
        task.wait(0.17)
        TS:Create(frame_3, TweenInfo.new(0.25), { BackgroundTransparency = 1 }):Play()
        TS:Create(uistroke, TweenInfo.new(0.24), { Transparency = 1 }):Play()
        task.wait(0.05)
        TS:Create(frame_2, TweenInfo.new(0.2), { Size = UDim2.new(0, 100, 0, 0) }):Play()
        task.wait(0.2)
        frame_2:Destroy()

        -- Remove from active list and reposition remaining
        for i, notif in ipairs(activeNotifs) do
            if notif == frame_2 then
    ...
    task.delay(tonumber(timee) or 10, close_notif)
end
