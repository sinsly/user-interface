...
init.Ascii = "qwertyuiopasdfghjklzxcvbnmQWRTYUIOPASDFGHJKLZXCVBNM";
init.GLOBAL_ENVIRONMENT = {};
init.Windows = {};
init.FontSemiBold = Font.new('rbxasset://fonts/families/GothamSSm.json',Enum.FontWeight.SemiBold,Enum.FontStyle.Normal);
init.Flags = {};
init.Colors = {
...
function init:IsMobile() : boolean
	return UserInputService.TouchEnabled;	
end;

function init:RandomString() : string
...
function init:GetTextSize(Text : TextLabel,CustomFont: Enum.Font) : Vector2
...
function init:IsMouseOverFrame(Frame : Frame) : boolean
...
function init:GetCalculatePosition(planePos: number, planeNormal: Vector3, rayOrigin: number, rayDirection: Vector3) : number
...
function init:CreateHover(Element : Frame,Callback : (boolean) -> any)
...
function init:GetIcon(name : string) : string
	return init.Lucide['lucide-'..tostring(name)] or init.Lucide[name] or init.Lucide[tostring(name)] or "";
end;

function init:SetIcon(name : string, value : string)
	init.Lucide[name] = value;
end;

function init:Rounding(num: number, numDecimalPlaces: number) : number
	local mult: number = 10 ^ (numDecimalPlaces or 0);
	return math.floor(num * mult + 0.5) / mult;
end;

function init:CreateAnimation(Instance: Instance , Time: number , Style : Enum.EasingStyle , Property : {[string] : any}) : Tween
...
function init:NewInput(Frame : Frame , Callback : () -> ()) : TextButton
...
function init:Drag(InputFrame: Frame, MoveFrame: Frame, Speed : number)
	...
	local function updateInput(input)
		...
		init:CreateAnimation(MoveFrame,Speed,nil,{
			Position = position
		});
	end;

	InputFrame.InputBegan:Connect(function(input)
		if (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) and #init.DragBlacklist <= 0 then 
			dragToggle = true
	...
	UserInputService.InputChanged:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch and #init.DragBlacklist <= 0 then
			if dragToggle then
				updateInput(input)
			end
		else
			if #init.DragBlacklist > 0 then
				dragToggle = false
			end
		end
	end);
...
function init:ScrollSignal(Scroll: ScrollingFrame,UIListLayout: UIListLayout,Type:string)
	if Type == 'X' then
...
function init:CreateResponse(args: {[string] : (any) -> any})
	local main = {};
...
function init:GetWindowFromElement(Element: GuiObject)
	for i,v in next , init.Windows do
...
function init:CreateOption(OptionButton: ImageButton): Elements
	init:CreateHover(OptionButton,function(bool)
		if bool then
			init:CreateAnimation(OptionButton,0.5,{
	...
	local OwnWindow = init:GetWindowFromElement(OptionButton);
	...
	DropShadow.Name = init:RandomString()
	...
	SpaceBox.Name = init:RandomString()
	...
	local ToggleExt = function(bool)
		if bool then
			...
			init:CreateAnimation(ExtElementFrame,0.45,{
				Size = UDim2.new(0, 200, 0, size)
			})

			init:CreateAnimation(DropShadow,0.45,{
				ImageTransparency = 0.750
			})

			init:CreateAnimation(UIStroke,0.45,{
				Transparency = 0
			})
			...
			SPAWN_THREAD = task.spawn(function()
				while true do task.wait(0.1)
					...
					init:CreateAnimation(ExtElementFrame,0.35,{
						Position = ud
					});
				end;
			end)
	...
	UserInputService.InputBegan:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 then
			if not init:IsMouseOverFrame(ExtElementFrame) and not init.GLOBAL_ENVIRONMENT.IS_HOLD_COLOR_PICKER then
				ToggleExt(false);
			end;
		end;
	end);

	local UIElements: Elements = init:CreateElements(ScrollingFrame,ScrollingFrame.ZIndex,Bindable);

	return UIElements;
end;

function init:CreateColorPicker(ColorBox: Frame,Transparency, Callback)
	Transparency = Transparency or 0;
	Callback = Callback or function() end;

	local OwnWindow = init:GetWindowFromElement(ColorBox);
	...
	local VisibleToggle = function(value)
		if value then
			...
			init:CreateAnimation(ColorPickBox,0.45,{
				ImageTransparency = 0,
				BackgroundTransparency = 0
			});

			init:CreateAnimation(MouseMovement,0.45,{
				ImageTransparency = 0,
			});

			init:CreateAnimation(UIStroke_2,0.45,{
				Transparency = 0
			});

			init:CreateAnimation(ColorRedGreenBlue,0.45,{
				BackgroundTransparency = 0
			});

			init:CreateAnimation(ColorRGBSlide,0.45,{
				BackgroundTransparency = 0
			});

			init:CreateAnimation(UIStroke_3,0.45,{
				Transparency = 0.75
			});

			init:CreateAnimation(ColorOpc,0.45,{
				BackgroundTransparency = 0
			});

			init:CreateAnimation(ColorOptSlide,0.45,{
				BackgroundTransparency = 0
			});

			init:CreateAnimation(UIStroke_4,0.45,{
				Transparency = 0.75
			});

			init:CreateAnimation(UIStroke_5,0.45,{
				Transparency = 0
			});

			init:CreateAnimation(PasteButton,0.45,{
				ImageTransparency = 0.45
			});

			init:CreateAnimation(CopyButton,0.45,{
				ImageTransparency = 0.45
			});

			init:CreateAnimation(hexCode,0.45,{
				BackgroundTransparency = 0.4
			});

			init:CreateAnimation(HexCodeText,0.45,{
	...
	ColorPickerFrame.Name = init:RandomString()
	...
	init:AddDragBlacklist(ColorPickerFrame);
	...
	ColorPickBox.Name = init:RandomString()
	...
	MouseMovement.Name = init:RandomString()
	...
	ColorRedGreenBlue.Name = init:RandomString()
	...
	ColorRGBSlide.Name = init:RandomString()
	...
	ColorOpc.Name = init:RandomString()
	...
	ColorOptSlide.Name = init:RandomString()
	...
	ColorOpt.Name = init:RandomString()
	...
	PasteButton.Name = init:RandomString()
	...
	CopyButton.Name = init:RandomString()
	...
	hexCode.Name = init:RandomString()
	hexCode.Parent = ColorOpt
	hexCode.BackgroundColor3 = init.Colors.Black
	...
	HexCodeText.Name = init:RandomString()
	...
	HexCodeText.FontFace = init.FontSemiBold
	...
	init:CreateHover(CopyButton,function(bool)
		if bool then
			init:CreateAnimation(CopyButton,0.45,{
	...
	init:CreateHover(PasteButton,function(bool)
		if bool then
			init:CreateAnimation(PasteButton,0.45,{
				ImageTransparency = 0.1
			})
	...
	PasteButton.MouseButton1Click:Connect(function()
		if init.GLOBAL_ENVIRONMENT.COLOR_COPY then
			local h,s,v = init.GLOBAL_ENVIRONMENT.COLOR_COPY.RGB:ToHSV();

			Transparency = init.GLOBAL_ENVIRONMENT.COLOR_COPY.OPC;

			OldCode = h;
			CodeH = s;
			CodeV = v;

			HexCodeText.Text = "#" .. tostring(init.GLOBAL_ENVIRONMENT.COLOR_COPY.RGB:ToHex());
			...
			ColorOpc.BackgroundColor3 = init.GLOBAL_ENVIRONMENT.COLOR_COPY.RGB;
	...
	CopyButton.MouseButton1Click:Connect(function()
		init.GLOBAL_ENVIRONMENT.COLOR_COPY = {
	...
	init:NewInput(ColorBox,function()
		VisibleToggle(true);
	end);
	...
	do
		...
		ColorPickerFrame.InputBegan:Connect(function(Input)
			if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
				...
				init.GLOBAL_ENVIRONMENT.IS_HOLD_COLOR_PICKER = true;
		...
		ColorPickerFrame.InputEnded:Connect(function(Input)
			if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
				IsPressM1 = false;
				init.GLOBAL_ENVIRONMENT.IS_HOLD_COLOR_PICKER = false;
		...
		UserInputService.InputBegan:Connect(function(Input)
			if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
				if not init:IsMouseOverFrame(ColorPickerFrame) then
					VisibleToggle(false);
				end;
			end;
		end)
		...
		ColorRedGreenBlue.InputBegan:Connect(function(Input)
			if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
				...
				while (UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) or IsPressM1) do task.wait()
					...
					init:CreateAnimation(ColorBox,0.5,{
						BackgroundColor3 = Color
					});

					init:CreateAnimation(ColorOpc,0.35,{
						BackgroundColor3 = Color
					});

					init:CreateAnimation(ColorPickBox,0.5,{
		...
		ColorPickBox.InputBegan:Connect(function(Input)
			if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
				...
				while (UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) or IsPressM1) do task.wait();
					...
					init:CreateAnimation(MouseMovement,0.2,nil,{
						Position = UDim2.new(CodeH, 0, 1 - CodeV, 0)
					})
				end
			end
...
function init:RandomStr(len: number): string
	local main = "";

	for i = 1 , len do
		local max = #init.Ascii;
		local rand = math.random(1,max);

		main = main .. init.Ascii:sub(rand,rand);
	end;

	return main;
end;

function init:AddDragBlacklist(Frame: Frame)
	Frame.InputBegan:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.MouseMovement then
			local finder = table.find(init.DragBlacklist , Frame);

			if not finder then
				table.insert(init.DragBlacklist , Frame);
			end;
		end;
	end);
	...
	Frame.InputEnded:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.MouseMovement then
			...
			if finder then
				table.remove(init.DragBlacklist , finder);
			end;
		end;
	end);
end;

function init:ProtectText(Label: TextLabel,Text: string)
	Label.RichText = true;

	local MainText = string.gsub(Text,'.',function(t)
		if not string.find(t,"[<>()\"']") then
			return string.format('<font %s="%s">%s</font>',init:RandomStr(5),init:RandomStr(5),t)
		end;
		return t;
	end);
...
function init:CreateDropdown(Parent: Frame, Default: string | {[string]: boolean}, Multiplier: boolean, AutoUpdate: boolean,Callback: (data: any) -> any)
	local Window = init:GetWindowFromElement(Parent);
	local Data = {};
	...
	init:AddDragBlacklist(DropdownItemFrame);
		...
		if value then
			...
			SPAWN_THREAD = task.spawn(function()
				while true do task.wait()
					...
					init:CreateAnimation(DropdownItemFrame,0.35,{
						Size = UDim2.new(0, 175, 0, math.clamp(baseSize,0,200))
					})
				end;
			end)

			init:CreateAnimation(UIStroke,0.35,{
				Transparency = 0
			})

			init:CreateAnimation(DropShadow,0.35,{
				ImageTransparency = 0.75
			})
		else
			if SPAWN_THREAD then
	...
	DropdownItemFrame.Name = init:RandomString()
	...
	local new_button = function()
		...
		db_selected.Name = init:RandomString()
		...
		db_selected.FontFace = init.FontSemiBold
		db_selected.TextColor3 = init.Colors.Main
			...
			for i,v in next , Data do
				...
				if Multiplier then
					if (typeof(Default) == 'table' and (Default[v] or table.find(Default,v))) or Default == v then
						Selected[v] = true
						bth.TextColor3 = init.Colors.Main;
					else
	...
	UserInputService.InputBegan:Connect(function(Input)
		if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
			if not init:IsMouseOverFrame(DropdownItemFrame) then
				func(false);
				Toggle(false);
			end;
		end;
...
function init:CreateElements(Parent : Frame , ZIndex : number , Event : BindableEvent,SearchAPI: {Path: string,Memory : (name:string) -> any}) : Elements
	local elements = {};
	local FatalWindow = init:GetWindowFromElement(Parent);
	...
	function elements:AddToggle(Config: Toggle)
		...
		init:AddDragBlacklist(Toggle);

		Toggle_Name.Name = init:RandomString()
		...
		Toggle_Name.FontFace = init.FontSemiBold;
		...
		init:ProtectText(Toggle_Name,Config.Name);

		ValueFrame.Name = init:RandomString()
		ValueFrame.Parent = Toggle
		ValueFrame.AnchorPoint = Vector2.new(1, 0.5)
		ValueFrame.BackgroundColor3 = init.Colors.Black
		...
		ValueIcon.Name = init:RandomString()
		...
		OptionButton.Name = init:RandomString()
		...
		local toggleImg = function(value)
			if value then
				init:CreateAnimation(ValueIcon,0.45,{
					ImageTransparency = 0,
					ImageColor3 = init.Colors.Main,
		...
		local OpcToggle = function(value)
			if value then
				...
				init:CreateAnimation(OptionButton,0.45,{
					ImageTransparency = (Config.Option and 0.600) or 1
				})

				toggleImg(Config.Default)

				init:CreateAnimation(ValueFrame,0.45,{
					BackgroundTransparency = 0,
				})

				init:CreateAnimation(Toggle_Name,0.45,{
		...
		init:CreateHover(ValueFrame,function(b)
		...
		init:NewInput(ValueFrame,function()
			Config.Default = not Config.Default;
			toggleImg(Config.Default);
			Config.Callback(Config.Default)
		end);

		local Respons = init:CreateResponse({
			Rename = function(new_name)
				Toggle_Name.Text = new_name;
				init:ProtectText(Toggle_Name,new_name);
			end,
			GetValue = function()
			...
			Option = (Config.Option and init:CreateOption(OptionButton)) or nil;
		});

		if Config.Flag then
			init.WindowFlags[FatalWindow][Config.Flag.."Toggle"] = Respons;
		end;
	...
	function elements:AddSlider(Config: Slider)
		...
		init:AddDragBlacklist(Slider);

		Slider_Name.Name = init:RandomString()
		...
		Slider_Name.FontFace = init.FontSemiBold
		...
		boxli.Name = init:RandomString()
		boxli.Parent = ValueFrame
		boxli.BackgroundColor3 = init.Colors.Main
		...
		ValueText.Name = init:RandomString()
		...
		ValueText.FontFace = init.FontSemiBold
		...
		local OpcToggle = function(value)
			if value then
				init:CreateAnimation(boxli,0.45,{
					BackgroundTransparency = 0,
				})
				...
				init:CreateAnimation(Slider_Name,0.45,{
					TextTransparency = 0.200
				})

				init:CreateAnimation(ValueText,0.45,{
		...
		local function update(Input)
			...
			local Value = init:Rounding(Main,Config.Round);
			...
			ValueFrame.InputEnded:Connect(function(Input)
				if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
					if UserInputService.TouchEnabled then
						if not init:IsMouseOverFrame(ValueFrame) then
							IsHold = false
						end;
					else
						IsHold = false
		...
		if Config.Flag then
			init.WindowFlags[FatalWindow][Config.Flag.."Slider"] = Respons;
		end;
	...
	function elements:AddButton(Config: Button)
		...
		Button.Name = init:RandomString()
		Button.Parent = Parent
		Button.BackgroundColor3 = init.Colors.Black
		Button.BorderColor3 = Color3.fromRGB(0, 0, 0)
		Button.BorderSizePixel = 0
		Button.Size = UDim2.new(1, -25, 0, 25)
		Button.ZIndex = ZIndex + 1
		init:AddDragBlacklist(Button);

		Button_Name.Name = init:RandomString()
		...
		Button_Name.FontFace = init.FontSemiBold
		...
		init:ProtectText(Button_Name,Config.Name);
		...
		local OpcToggle = function(value)
			if value then
				init:CreateAnimation(Button_Name,0.45,{
					TextTransparency = 0.4,
				})

				init:CreateAnimation(Button,0.45,{
					BackgroundTransparency = 0,
				})
			else
				init:CreateAnimation(Button_Name,0.45,{
		...
		init:CreateHover(Button,function(value)
			if value then
				init:CreateAnimation(Button,0.45,{
				...
				init:CreateAnimation(Button,0.45,{
					BackgroundColor3 = init.Colors.Black
				})
			end;
		end)

		init:NewInput(Button,function()
			Config.Callback();
		end)

		return init:CreateResponse({
			Rename = function(new_name)
				Button_Name.Text = new_name
				init:ProtectText(Button_Name,new_name);
			end,
			GetValue = function()
				return Config.Default;
			end,
	...
	function elements:AddColorPicker(Config: ColorPicker)
		...
		ColorPicker.Name = init:RandomString()
		...
		init:AddDragBlacklist(ColorPicker);

		ColorPicker_Name.Name = init:RandomString()
		...
		ColorPicker_Name.FontFace = init.FontSemiBold
		...
		local res = init:CreateColorPicker(ValueFrame,Config.Transparency,function(rgb,opc)
		...
		local OpcToggle = function(value)
			if value then
				init:CreateAnimation(ColorPicker_Name,0.45,{
		...
		if Config.Flag then
			init.WindowFlags[FatalWindow][Config.Flag.."ColorPicker"] = Respons;
		end;
	...
	function elements:AddDropdown(Config: Dropdown)
		...
		Dropdown.Name = init:RandomString()
		...
		init:AddDragBlacklist(Dropdown);

		Dropdown_Name.Name = init:RandomString()
		...
		Dropdown_Name.FontFace = init.FontSemiBold
		...
		init:ProtectText(Dropdown_Name,Config.Name);
		...
		Value_Text.Name = init:RandomString()
		...
		Value_Text.FontFace = init.FontSemiBold
		...
		local res;
		res = init:CreateDropdown(ValueFrame,Config.Default,Config.Multi,Config.AutoUpdate,function(args)
			Config.Default = args;
			Value_Text.Text = DataParser(Config.Default);
		...
		res:on_toggle(function(b)
			if b then
				init:CreateAnimation(icon,0.35,{
					Rotation = -180,
					ImageColor3 = init.Colors.Main
				})
			else
		...
		local OpcToggle = function(value)
			if value then
				init:CreateAnimation(Dropdown_Name,0.45,{
					TextTransparency = 0.2,
				})
				...
				init:CreateAnimation(icon,0.45,{
					ImageTransparency = 0
				})

				init:CreateAnimation(Value_Text,0.45,{
		...
		local Respons = init:CreateResponse({
			Rename = function(new_name)
				...
				init:ProtectText(Dropdown_Name,new_name);
			end,
			GetValue = function()
				return Config.Default;
			end,
		...
		if Config.Flag then
			init.WindowFlags[FatalWindow][Config.Flag.."Dropdown"] = Respons;
		end;
	...
	game:GetService("UserInputService").InputBegan:Connect(function(input, processed)
		...
		Keybind.Name = init:RandomString()
		...
		init:AddDragBlacklist(Keybind);

		Keybind_Name.Name = init:RandomString()
		...
		Keybind_Name.FontFace = init.FontSemiBold
		...
		init:ProtectText(Keybind_Name,Config.Name);
		...
		local OpcToggle = function(value)
			if value then
				init:CreateAnimation(Keybind_Name,0.45,{
					TextTransparency = 0.2,
				})
		...
		local Respons = init:CreateResponse({
			Rename = function(new_name)
				...
				init:ProtectText(Keybind_Name,new_name);
			end,
			GetValue = function()
				return Config.Default;
			end,
		...
		if Config.Flag then
			...
			init.WindowFlags[FatalWindow][Config.Flag.."Keybind"] = Respons;
		end;
...
function init:CreateConfigWindow(Root: ScreenGui , Fatal , Button: ImageButton)
	...
	init:AddDragBlacklist(ConfigWindowFrame);
	...
	local ElementToggle = function(value)
		...
		if value then
			...
			init:CreateAnimation(ConfigWindowFrame,0.3,{
				Size = UDim2.new(0, 185, 0, 195),
			})

			init:CreateAnimation(UIStroke,0.3,{
				Transparency = 0
			})

			init:CreateAnimation(DropShadow,0.3,{
				ImageTransparency = 0.750
			})

			init:CreateAnimation(ScrollingFrame,0.3,{
				BackgroundTransparency = 0
			})

			init:CreateAnimation(UIStroke_2,0.3,{
				Transparency = 0.750
			})

			init:CreateAnimation(UIStroke_3,0.3,{
				Transparency = 0.750
			})

			init:CreateAnimation(SpaceBox,0.3,{
				BackgroundTransparency = 0
			})

			init:CreateAnimation(InputFrame,0.3,{
				BackgroundTransparency = 0
			})

			init:CreateAnimation(TextBox,0.3,{
				TextTransparency = 0.5
			})

			init:CreateAnimation(AddConfigButton,0.3,{
				ImageTransparency = 0.500
			})

			init:CreateAnimation(GetConfigButton,0.3,{
				ImageTransparency = 0.500
			})

			init:CreateAnimation(SaveConfigButton,0.3,{
				ImageTransparency = 0.500
			})

			init:CreateAnimation(DeleteButton,0.3,{
			...
			init:CreateAnimation(ConfigWindowFrame,0.275,{
	...
	ScrollingFrame.Active = true
	ScrollingFrame.BackgroundColor3 = init.Colors.Black
	ScrollingFrame.BorderColor3 = Color3.fromRGB(0, 0, 0)
	ScrollingFrame.BorderSizePixel = 0
	...
	InputFrame.BackgroundColor3 = init.Colors.Black
	...
	TextBox.ZIndex = 209
	TextBox.FontFace = init.FontSemiBold
	TextBox.Text = ""
	...
	DeleteButton.ImageColor3 = init.Colors.Main
	...
	init:CreateHover(DeleteButton,function(bool)
		if bool then
			init:CreateAnimation(DeleteButton,0.3,{
				ImageTransparency = 0;
			})
	...
	init:CreateHover(SaveConfigButton,function(bool)
		if bool then
			init:CreateAnimation(SaveConfigButton,0.3,{
	...
	init:CreateHover(GetConfigButton,function(bool)
		if bool then
			init:CreateAnimation(GetConfigButton,0.3,{
				ImageTransparency = 0.15;
			})
	...
	init:CreateHover(AddConfigButton,function(bool)
		if bool then
			init:CreateAnimation(AddConfigButton,0.3,{
				ImageTransparency = 0.15;
			})
	...
	game:GetService('UserInputService').InputBegan:Connect(function(Input,Typing)
		if Input.UserInputType == Enum.UserInputType.Touch or Input.UserInputType == Enum.UserInputType.MouseButton1 then
			if UIToggle and not init:IsMouseOverFrame(ConfigWindowFrame) then
				ElementToggle(false);
			end;
		end;
	end);
	...
	res = init:CreateResponse({
		...
		Refresh = function()
			...
			for i,v in next, Configs do
				...
				if tostring(import.Name) == tostring(v) or import.Name == v then
					...
					import.Name = v;
					import.Element.TextColor3 = init.Colors.Main;
					button.Text = "  "..tostring(v);
				end;
				...
				button.MouseButton1Click:Connect(function()
					...
					import.Element = button;
					import.Name = v;
					button.TextColor3 = init.Colors.Main;
					button.Text = "  "..tostring(v);
				end);
			end;
		end,
		...
		Init = function(Name: string,Folder: string)
			if not isfolder(Folder or "init") then
				makefolder(Folder or "init");	
			end;

			if not isfolder((Folder or "init").."/Config") then
				makefolder((Folder or "init").."/Config");	
			end;

			local cfgPath = (Folder or "init").."/Config/"..tostring(Name);
...
function init.new(Window: Window)
	Window = Window or {};
	Window.Name = Window.Name or "init";
	...
	Fatal.Notifier = init.__NOTIFIER_CACHE or init:CreateNotifier();

	local initwin = Instance.new("ScreenGui")
	...
	init.WindowFlags[initwin] = {};

	init:ScrollSignal(tbc,UIListLayout,'X');
	...
	FatalFrame:GetPropertyChangedSignal("BackgroundTransparency"):Connect(function()
		if FatalFrame.BackgroundTransparency >= 0.99 then
			initwin.Enabled = false;
		else
			initwin.Enabled = true;
		end;
	end);
	...
	local ToggleUI = function(bool)
		...
		if bool then
			...
			init:CreateAnimation(FatalFrame,0.15,{
				Size = Window.Scale,
				BackgroundTransparency = 0,
			})

			init:CreateAnimation(Header,0.5,{
				BackgroundTransparency = 0,
			})

			init:CreateAnimation(HeaderLine,0.5,{
				BackgroundTransparency = 0,
			})

			init:CreateAnimation(Bottom,0.5,{
				BackgroundTransparency = 0,
			})

			init:CreateAnimation(HeaderLine_2,0.5,{
				BackgroundTransparency = 0,
			})

			init:CreateAnimation(HeaderLineShadow,0.5,{
				BackgroundTransparency = 0.5,
			})

			init:CreateAnimation(HeaderLineShadow_2,0.5,{
				BackgroundTransparency = 0.5,
			})

			init:CreateAnimation(InfoButton,0.25,{
				ImageTransparency = 0.5
			})

			init:CreateAnimation(SearchButton,0.25,{
				ImageTransparency = 0.5
			})

			init:CreateAnimation(SaveButton,0.25,{
				ImageTransparency = 0.5
			})

			init:CreateAnimation(HeaderText,0.35,{
				TextStrokeTransparency = 0.640,
				TextTransparency = 0
			})

			init:CreateAnimation(UserIcon,0.45,{
				ImageTransparency = 0,
				Position = UDim2.new(1, -10,0.5, 0)
			})

			init:CreateAnimation(User_name,0.35,{
				TextTransparency = 0,
				Position = UDim2.new(1, -40,0, 3)
			})

			init:CreateAnimation(expire_days,0.5,{
				TextTransparency = 0,
			...
			init:CreateAnimation(InfoButton,0.1,Enum.EasingStyle.Back,{
				Position = UDim2.new(1, -5,0.5, 0)
			})

			init:CreateAnimation(SearchButton,0.1,Enum.EasingStyle.Back,{
				Position = UDim2.new(0,10,0.5, 0)
			})

			init:CreateAnimation(SaveButton,0.1,Enum.EasingStyle.Back,{
			...
			table.clear(init.DragBlacklist);

			for i,v in next , Fatal.Menus do
				v.ValueSelect(false);
			end;

			init:CreateAnimation(SaveButton,0.35,{
				Position = UDim2.new(0,35,1, 20)
			})

			init:CreateAnimation(SearchButton,0.35,{
				Position = UDim2.new(0,10,1, 20)
			})

			init:CreateAnimation(InfoButton,0.35,{
			...
			init:CreateAnimation(UserIcon,0.35,{
				ImageTransparency = 1,
				Position = UDim2.new(1, -10,0.5, 0)
			})
			...
			init:CreateAnimation(expire_days,0.1,{
				TextTransparency = 1,
				Position = UDim2.new(1, -40,0, 16)
			})
			...
			init:CreateAnimation(FatalFrame,0.75,{
				BackgroundTransparency = 1,
			})
	...
	initwin.Name = init:RandomString();
	initwin.Parent = CoreGui;
	initwin.ResetOnSpawn = false;
	initwin.IgnoreGuiInset = true;
	initwin.ZIndexBehavior = Enum.ZIndexBehavior.Global;

	table.insert(init.Windows,initwin)

	protect_gui(initwin);

	FatalFrame.Active = true;
	FatalFrame.Name = init:RandomString()
	FatalFrame.Parent = initwin
	...
	HeaderLine.Name = init:RandomString()
	...
	HeaderText.Name = init:RandomString()
	...
	MenuButtonCont.Name = init:RandomString()
	...
	UserProfle.Name = init:RandomString()
	...
	UserIcon.Name = init:RandomString()
	...
	User_name.Name = init:RandomString()
	...
	expire_days.Name = init:RandomString()
	...
	HeaderLineShadow.Name = init:RandomString()
	...
	MenuFrame.Name = init:RandomString()
	...
	HeaderLine_2.Name = init:RandomString()
	...
	HeaderLineShadow_2.Name = init:RandomString()
	...
	init:Drag(FatalFrame,FatalFrame,0.1);
	...
	function Fatal:GetFlags()
		return init.WindowFlags[initwin];
	end;
	...
	function Fatal:AddConfig()
		...
		init:CreateHover(ConfigButton,function(bool)
			if bool then
				init:CreateAnimation(ConfigButton,0.3,{
					ImageTransparency = 0.2500;
				})
			else
				init:CreateAnimation(ConfigButton,0.3,{
		...
		return init:CreateConfigWindow(initwin,Fatal,ConfigButton);
	end;
	...
	function Fatal:LoadConfig(config)
		...
		for i,v in next , config do
			if i ~= "Info" then
				local Element = init.WindowFlags[initwin][i];
	...
	function Fatal:AddMenu(Menu : Menu)
		...
		MenuButton.Name = init:RandomString()
		...
		Icon.SizeConstraint = Enum.SizeConstraint.RelativeYY
		Icon.ZIndex = 5
		Icon.Image = init:GetIcon(Menu.Icon);
		Icon.ImageColor3 = init.Colors.Main
		Icon.ScaleType = Enum.ScaleType.Crop

		UICorner_2.CornerRadius = UDim.new(1, 0)
		UICorner_2.Parent = Icon

		menu_name.Name = init:RandomString()
		...
		local text_size = init:GetTextSize(menu_name);
		...
		MenuLiber.Name = init:RandomString()
		...
		BindEvent.Name = init:RandomString();

		if not Menu.AutoFill then
			init:ScrollSignal(Right,UIListLayout_3,'Y');
			init:ScrollSignal(Center,UIListLayout_2,'Y');
			init:ScrollSignal(Left,UIListLayout,'Y');
		else
		...
		init:CreateHover(MenuButton,function(bool)
		...
		init:NewInput(MenuButton,function()
			for i,v in next , Fatal.Menus do
		...
		function MenuLib:AddPreview(Config: Preview)
			...
			PreviewName.FontFace = init.FontSemiBold
			...
			local Toggle = function(v)
				if v then
					init:CreateAnimation(Preview,0.5,{
						BackgroundTransparency = 0
					})

					init:CreateAnimation(PreviewName,0.5,{
						TextStrokeTransparency = 0.750,
						TextTransparency = 0
					})
					
					init:CreateAnimation(Main,0.5,{
						BackgroundTransparency = 0
					})
					...
					init:CreateAnimation(MainBlock,0.5,{
		...
		function MenuLib:AddListBox(Config : Listbox)
			...
			ListBox.Name = init:RandomString()
			...
			init:AddDragBlacklist(ScrollingFrame);

			Elements.Name = init:RandomString()
			...
			SearchBox.Name = init:RandomString()
			SearchBox.Parent = Elements
			SearchBox.BackgroundColor3 = init.Colors.Black
			...
			TextBox.BackgroundColor3 = init.Colors.Black
			TextBox.BackgroundTransparency = 1.000
			TextBox.BorderColor3 = Color3.fromRGB(0, 0, 0)
			TextBox.BorderSizePixel = 0
			...
			ListName.Name = init:RandomString()
			...
			ListName.ZIndex = 10
			ListName.FontFace = init.FontSemiBold
			...
			local Toggle = function(v)
				if v then
					init:CreateAnimation(ListBox,0.5,{
						BackgroundTransparency = 0
					})
					...
					init:CreateAnimation(UIStroke_2,0.5,{
						Transparency = 0.75
					})

					init:CreateAnimation(UIStroke_3,0.5,{
						Transparency = 0.75
					})

					init:CreateAnimation(SearchBox,0.5,{
						BackgroundTransparency = 0
					})

					init:CreateAnimation(TextBox,0.5,{
						TextTransparency = 0
					})
					...
					init:CreateAnimation(ScrollingFrame,0.45,{
						BackgroundTransparency = 0,
					...
					table.foreach(ScrollingFrame:GetChildren(),function(i,v)
						if v:IsA('TextButton') then
							init:CreateAnimation(v,0.45,{
								TextTransparency = 0.5
							});
						end;
					end);

					init:CreateAnimation(ListName,0.5,{
				...
				Refresh = function()
					...
					for i,v in next , Config.Values do
						...
						if Config.Multi then
							...
							init:CreateHover(bth,function(bool)
								if not Config.Default[v] then
									if bool then
										init:CreateAnimation(bth,0.25,{
											TextTransparency = 0;
										})
									else
										init:CreateAnimation(bth,0.25,{
			...
			if Config.Flag then
				init.WindowFlags[initwin][Config.Flag.."ListBox"] = res;
			end;
		...
		function MenuLib:AddSection(Config : Section)
			...
			local Toggle = function(v)
				if v then
					init:CreateAnimation(Section,0.5,{
						BackgroundTransparency = 0
					})
					...
					init:CreateAnimation(SectionName,0.5,{
						TextStrokeTransparency = 0.750,
						TextTransparency = 0
					})
				else
			...
			Section.Name = init:RandomString()
			...
			SectionName.Name = init:RandomString()
			...
			SectionName.FontFace = init.FontSemiBold;
			...
			UIListLayout:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
				...
				if not Menu.AutoFill then
					init:CreateAnimation(Section,0.25,{
			...
			return init:CreateElements(Elements,Elements.ZIndex,BindEvent,{
				Path = Menu.Name .. " > ".. Config.Name,
	...
	do
		InfoButton.Name = init:RandomString()
		...
		SaveButton.Name = init:RandomString()
		...
		init:CreateHover(SaveButton,function(bool)
			if bool then
				init:CreateAnimation(SaveButton,0.5,{
					ImageTransparency = 0.1
				})
			else
				init:CreateAnimation(SaveButton,0.5,{
		...
		init:CreateHover(InfoButton,function(bool)
			if bool then
				init:CreateAnimation(InfoButton,0.5,{
					ImageTransparency = 0.1
				})
			else
				init:CreateAnimation(InfoButton,0.5,{
	...
	do
		...
		init:AddDragBlacklist(SearchFrame);
		...
		local SearchToggle = function(value)
			if value then
				...
				init:CreateAnimation(SearchFrame,0.35,{
					Size = searchScale
				})
				...
				init:CreateAnimation(SearchBox,0.35,{
					BackgroundTransparency = 0
				})
		...
		SearchFrame.Name = init:RandomString()
		SearchFrame.Parent = initwin;
		...
		init:CreateHover(SearchButton,function(bool)
			if bool then
				init:CreateAnimation(SearchButton,0.5,{
					ImageTransparency = 0.1
				})
			else
				init:CreateAnimation(SearchButton,0.5,{
		...
		local get_button = function(Name,Path,TAB_WARP)
			...
			ResultFrame.Name = init:RandomString()
			ResultFrame.Parent = ScrollingFrame;
			ResultFrame.BackgroundColor3 = init.Colors.Black
			...
			FeatureName.Name = init:RandomString()
			...
			FeatureName.FontFace = init.FontSemiBold
			...
			FeaturePath.Name = init:RandomString()
			...
			FeaturePath.FontFace = init.FontSemiBold
			...
			local button = init:NewInput(ResultFrame);

			init:CreateHover(button,function(bool)
				if bool then
					init:CreateAnimation(ResultFrame,0.5,{
						BackgroundTransparency = 0
					})
				else
					init:CreateAnimation(ResultFrame,0.5,{
		...
		SearchButton.MouseButton1Click:Connect(function()
			...
			table.foreach(Fatal.ElementContents,function(i,v)
				...
				SearchInformation[v.Path.." - "..init:RandomString()] = {
					root = button,
					callback = function()
		...
		UserInputService.InputBegan:Connect(function(input)
			if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
				if not init:IsMouseOverFrame(SearchFrame) then
					SearchToggle(false);
				end
			end
		end)
	...
	function Fatal:GetButton()
		...
		backpack.Name = "init"..init:RandomString();
		...
		backpack.MouseEnter:Connect(function()
			init:CreateAnimation(backpack,0.3,nil,{
				BackgroundColor3 = init.Colors.Main,
...
function init:Loader(Config: Loader)
	Config = Config or {};
	Config.Name = Config.Name or "init";
	...
	BlackFrame.Name = init:RandomString()
	...
	init:CreateAnimation(Blur,1,{
		Size = 60
	})

	init:CreateAnimation(BlackFrame,0.5,{
	...
	local createText = function(TEXT)
		...
		local textsize = init:GetTextSize(ASCII);
	...
	do
		...
		StartText.Name = init:RandomString()
		...
		local textsize = init:GetTextSize(StartText);
		...
		init:CreateAnimation(StartText,0.45,{
			TextTransparency = 0
		})

		init:CreateAnimation(UIScale,0.5,{
			Scale = Config.Scale;
		});

		task.wait(0.45);

		init:CreateAnimation(StartText,0.35,{
		...
		for i,v in next , PosText do
			if i > 1 then
				init:CreateAnimation(v.Text,0.65,{
					Position = UDim2.new(0.5,0,0.5,0);
					TextTransparency = 0
				})
			end;
		...
		init:CreateAnimation(StartText,1.5,{
			TextTransparency = 1
		})

		for i,v in next , PosText do
			init:CreateAnimation(v.Text,1.5,{
				TextTransparency = 1
			})
		end;

		init:CreateAnimation(Blur,1.5,{
			Size = 0
		})

		init:CreateAnimation(BlackFrame,1.5,{
			BackgroundTransparency = 1
		})
...
function init:CreateNotifier(): Notifier
	if init.__NOTIFIER_CACHE then return init.__NOTIFIER_CACHE; end;

	local Notify = Instance.new("ScreenGui")
	local layout = Instance.new("Frame")
	local UIListLayout = Instance.new("UIListLayout")

	Notify.Name = init:RandomString();
	Notify.Parent = CoreGui
	Notify.ResetOnSpawn = false
	Notify.ZIndexBehavior = Enum.ZIndexBehavior.Global
	Notify.IgnoreGuiInset = true;

	layout.Name = init:RandomString();
	...
	local res = init:CreateResponse({
		Notify = function(Config: Notify)
			...
			notify_block.Name = init:RandomString()
			...
			Icon.BackgroundColor3 = init.Colors.Main
			Icon.BackgroundTransparency = 1.000
			Icon.BorderColor3 = Color3.fromRGB(0, 0, 0)
			Icon.BorderSizePixel = 0
			Icon.Position = UDim2.new(0, 7, 0, 7)
			Icon.Size = UDim2.new(0, 18, 0, 18)
			Icon.Image = init:GetIcon(Config.Icon);
			Icon.ImageColor3 = init.Colors.Main
			...
			HeaderText.FontFace = init.FontSemiBold
			HeaderText.Text = Config.Title
			HeaderText.TextColor3 = init.Colors.Main
			HeaderText.TextSize = 13.000
			HeaderText.TextTransparency = 1
			HeaderText.TextXAlignment = Enum.TextXAlignment.Left

			BodyText.Name = init:RandomString()
			...
			BodyText.ZIndex = 55
			BodyText.FontFace = init.FontSemiBold
			...
			local updateScale = function()
				local TitleScale = init:GetTextSize(HeaderText,Enum.Font.GothamBold);
				local ContentScale = init:GetTextSize(BodyText,Enum.Font.GothamMedium);
				...
				init:CreateAnimation(notify_block,0.35,{
					Position = UDim2.new(0.5, 0, 0, 0)
				});
			end;

			task.delay(0.1,function()
				init:CreateAnimation(notify_block,0.25,{
					BackgroundTransparency = 0.1
				})

				init:CreateAnimation(UIStroke,0.25,{
					Thickness = 2.500,
					Transparency = 0.900
				})

				init:CreateAnimation(Icon,0.25,{
					ImageTransparency = 0
				})

				init:CreateAnimation(HeaderText,0.25,{
					TextTransparency = 0.200
				})

				init:CreateAnimation(BodyText,0.25,{
			...
			task.delay(Config.Duration + 0.25,function()
				...
				task.delay(0.35,function()
					init:CreateAnimation(notify,0.5,{
						Size = UDim2.new(0,0,0,0)
					})
	...
	init.__NOTIFIER_CACHE = res;

	return res;
end;

init.init_PID = init:RandomString();
