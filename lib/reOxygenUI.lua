			...
			function SectionTable:NewSlider(slider)
				...
				local function update(Input)
					local SizeScale = math.clamp(
						((Input.Position.X - MFrame.AbsolutePosition.X) / MFrame.AbsoluteSize.X),
						0, 1
					)
					local rawValue = ((slider.Max - slider.Min) * SizeScale) + slider.Min
					local factor = 10 ^ (slider.Steps or 0)
					local Value = math.floor(rawValue * factor + 0.5) / factor
					local Size = UDim2.fromScale(SizeScale, 1)
					local format = "%." .. tostring(slider.Steps or 0) .. "f"
					ValueText.Text = string.format(format, Value) .. "/" .. tostring(slider.Max)
				...
				return {
					...
					Value = function(lrm)
						local factor = 10 ^ (slider.Steps or 0)
						local Value = math.floor(lrm * factor + 0.5) / factor

						TFrame.Size = UDim2.new((Value / slider.Max), 0, 1, 0)
						ValueText.Text = string.format("%." .. tostring(slider.Steps or 0) .. "f", Value) .. "/" .. tostring(slider.Max)
...
function SectionTable:NewDropdown(drop)
    drop = Config(drop, {
        ...
        Default = 'Two',
        MultiSelect = false, -- NEW: enable multiple selection
    ...
    UIGradient.Rotation = 90
    UIGradient.Transparency = NumberSequence.new{NumberSequenceKeypoint.new(0,0), NumberSequenceKeypoint.new(0.84,0.25), NumberSequenceKeypoint.new(1,1)}
    UIGradient.Parent = TextInt
    ...
    UIGradient_2.Rotation = 90
    UIGradient_2.Transparency = NumberSequence.new{NumberSequenceKeypoint.new(0,0), NumberSequenceKeypoint.new(0.84,0.25), NumberSequenceKeypoint.new(1,1)}
    UIGradient_2.Parent = ValueText
    ...
    if not drop.MultiSelect then
        selectedValues = drop.Default
    end

    local function Updater(value)
        if drop.MultiSelect then
    ...
    Button.MouseButton1Click:Connect(function()
        ...
        currentDropdown = MFrame
    end)

    -- CLOSE DROPDOWN WHEN CLICKING OUTSIDE (but allow clicks on UI)
    ...
    UserInputService.InputBegan:Connect(function(input, gameProcessed)
        if input.UserInputType == Enum.UserInputType.MouseButton1 and currentDropdown then
            ...
            local guiPos = currentDropdown.AbsolutePosition
            local guiSize = currentDropdown.AbsoluteSize
